...
const displayOverlay = async (isDamage, tokens, isTargeted = false) => {
  ...
  if (TH_CONFIG.ENABLE_TOKEN_IMAGES){
    const content = await renderTemplate(
      ...
      {thumbnails, damageTypes, damageSubtypes, helpText, tokens},
    );
    // Render the dialog
...
Handlebars.registerHelper('upperCaseFirst', function (aString) {
  return aString.charAt(0).toUpperCase() + aString.slice(1);
})

Handlebars.registerHelper('concatLocal', function (...values) {
  values.pop();
  return values.join('');
})

Handlebars.registerHelper('ifSuperiorCustom', function(v1, v2, options) {
  if(v1 > v2) {
