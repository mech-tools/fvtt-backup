class ForceClientSettings {
  ...
  static settingsConfigDefaultOptions(wrapped, ...args) {
    return foundry.utils.mergeObject(wrapped(...args), { scrollY: [".tab.active .settings-list"] });
  }
  ...
  static async renderSettingsConfig(app, html) {
    ...
    const elem = html.find(".settings-list .form-group:not(.submenu)");
    elem.each(function () {
                  ...
                  [item.key]: {
                    ...
                    name:
                      ...
                      (`${item.namespace}.${item.key}` === "core.keybindings"
                        ? "FORCECLIENTSETTINGS.settings.forced-config.keybindings"
                        : ""),
