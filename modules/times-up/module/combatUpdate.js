...
export async function handlePreUpdateCombat(update, options, user) {
    ...
    for (let turn of this.turns) {
        ...
        let requiredEffects = turnActor.temporaryEffects?.filter(ef => ef.data.duration &&
            (ef.data.duration.rounds || ef.data.duration.turns)
            && !(ef.data.duration.startRound || ef.data.duration.startTurn));
        let ammendedEffects = requiredEffects.map(ef => {
            let efData = duplicate(ef.data);
    ...
    for (let checkTurn = startTurn; checkTurn <= currentTurn; checkTurn++) {
        ...
        if (dae) {
            ...
            turnActor.effects?.filter(async (ef) => {
                switch (getMacroRepeat(ef.data)) {
                    case "startEveryTurn":
                        if (checkTurn > startTurn)
                            dae.daeMacro("each", turnActor, ef.data, { actor: turnActor, effectId: ef.id, tokenId: turn.token.id, actorUuid: turnActor.uuid, actorID: turnActor.id, efData: ef.data });
                        break;
                    case "endEveryTurn":
        ...
        let expiredEffects = turn?.actor?.effects?.filter(ef => {
            let specialDuration = ef.data.flags?.dae?.specialDuration;
            if (!specialDuration || !(ef.data.flags.dae.specialDuration instanceof Array))
                specialDuration = [];
            const effectStartTurn = ef.data.duration.startRound * this.turns.length + ef.data.duration.startTurn;
        ...
        for (let targetTurn of this.turns) { //
            let expiredEffects = targetTurn?.actor?.effects?.filter(ef => {
                ...
                if (specialDuration.some(sd => ["turnStartSource", "turnEndSource"].includes(sd))) {
                    if (!ef.data.origin.startsWith(turn.actor?.uuid))
                        return false;
                    const effectStartTurn = ef.data.duration.startRound * this.turns.length + ef.data.duration.startTurn;
        ...
        Hooks.on("preDeleteCombat", (combat, options, userId) => {
            combat.turns.forEach(async (turn) => {
                ...
                let expiredEffects = turnActor.temporaryEffects?.filter(ef => {
                    return ((ef.data.duration?.rounds || ef.duration?.turns) && ef.duration?.remaining < 1);
                });
                ...
                let updatedEffects = turnActor.temporaryEffects?.filter(ef => ef.data.duration && ((ef.data.duration.rounds || ef.data.duration.turns) && ef.duration.remaining > 1)).map(ef => {
                    // TODO lookup seconds per round.
                    ef.data.duration.seconds = Math.floor(ef.data.duration.rounds) * CONFIG.time.roundTime;
                    ef.data.duration.rounds = 0;
                    ef.data.duration.turns = 0;
                    return ef.data;
                });
